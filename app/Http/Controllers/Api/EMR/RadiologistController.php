...
class RadiologistController extends Controller
{
    public function index()
    {
        return response()->json([
            ...
            'reports'       => Appointment::where('status', '<', 6)->with(['service', 'patient',])->orderBy('date', 'DESC')->orderBy('schedule', 'ASC')->paginate(10),
    ...
    public function reviews(){
        return response()->json([
            'applicants'    => User::whereIn('user_type', ['Patient', 'Both'])->orderBy('first_name', 'ASC')->with(['area', 'state',])->get(),
            'findings'      => RadFinding::orderBy('code', 'ASC')->get(), 
            'reports'       => Appointment::where('status', '>', 6)->with(['service', 'patient', 'report.findings', 'radiologist'])->orderBy('date', 'DESC')->orderBy('schedule', 'ASC')->paginate(10),
            'nations'       => Country::orderBy('name', 'ASC')->get(),   
            'patients'      => Patient::orderBy('last_name', 'ASC')->get(),
            'services'      => Service::orderBy('name', 'ASC')->get(),  
        ]);
