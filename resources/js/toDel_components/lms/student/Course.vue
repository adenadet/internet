<template>
    <div class="row">
        <div class="card col-lg-3 col-sm-6 col-md-4 d-flex align-items-stretch" v-for="(lesson, index) in lessons" :key="index">
            <div class="bg-light">
                ...
                <div class="card-body pt-0" style="height:100px;">
                    <div class="row">
                        <div class="col-9"><p class="text-muted text-sm">{{lesson.content | readMore(100, '...')}}</p></div>
                        <div class="col-3 text-center"><img src="" alt="" class="img-circle img-fluid"></div>
                    </div>
                </div>
                <div class="card-footer">
                    <div class="text-right" v-if="index > course.level">
                        <button title="Finish The Previous Course" class="btn btn-sm btn-danger" @click="prevent"><i class="fa fa-lock"></i> Locked</button>
                    </div>
                    <div class="text-right" v-else-if="index == 0 && course.level == null">
                    ...
                    <div class="text-right" v-else>
                        ...
                        <router-link v-else :to="'/student_area/lesson/'+lesson.id" title="Repeat Course" class="btn btn-sm btn-success"><i class="fa fa-circle-o"></i> Read Again</router-link>
                    </div>
                </div>
            </div>
        </div>
    ...
    methods:{
        ...
        getAllInitials(){
            ...
            axios.get('/api/lms/std_courses/'+this.$route.params.id).then(response =>{
                ...
                this.lessons = response.data.course[0].course.lessons;
        ...
        startCourse(id){
            ...
            .then(response =>{
                ...
                if (response.data.message == "Error"){
                    ...
                    this.$router.push('/student_area/courses');
                }
                else{
                    this.$router.push('/student_area/lesson/'+id);
                }
            })
            .catch(()=>{});
        },
