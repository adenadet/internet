...
export default {
    ...
    methods:{
        createOption(){
            this.optionData.post('/api/lms/options').then(response=>{
                ...
                Fire.$emit('optionRefresh', response); 
            })
            .catch(()=>{
                this.$Progress.fail();
                Swal.fire({
    ...
    mounted(){ 
        Fire.$on('optionDataFill', option =>{
            ...
            this.optionData.option_text = option.option_text;
            this.optionData.option_img = option.option_img;
            this.optionData.points = option.points;
        });
