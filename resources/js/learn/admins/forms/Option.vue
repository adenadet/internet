    ...
    methods:{
        createOption(){
            this.optionData.post('/api/lms/options').then(response=>{
                ...
                Swal.fire({icon: 'success', title: 'An Option was successfully added!',});
                this.$Progress.finish();
                this.optionData.reset();  
            })
            .catch(()=>{
                this.$Progress.fail();
                Swal.fire({icon: 'error', title: 'The Option was not added try again later!',});
            });
        },
        updateOption(){
            ...
            .catch(()=>{
                this.$Progress.fail();
                Swal.fire({icon: 'error', title: 'Option was not updated, try again later!',});
            });
        },
    },
    mounted(){ 
        Fire.$on('optionDataFill', relay =>{
            console.log(relay);
            this.question = relay.question;
            this.optionData.fill(relay.option);
        });
